<h2> Mingu Kang 🎃 </h2>

MS Student @ [MLIC Lab](https://mllab.unist.ac.kr/), UNIST AI<br>
Interested in **Multi-Agent Reinforcement Learning** — especially zero-shot coordination and robustness.<br>
Background in business & finance, with hands-on experience connecting domain knowledge to ML systems.<br>

📧 mingukang@unist.ac.kr

---

<h3> 🔬 Publications </h3>

| Title | Venue | Status |
| ------ | ------ | ------ |
| Shaping Zero-Shot Coordination via State Blocking | NeurIPS 2026 | Under Review |
| VERSA: Verified Event Data Format for Reliable Soccer Analytics | CKIM 2026 | Under Review |
| Interaction-Breaking Adversarial Learning Framework for Robust Multi-Agent Reinforcement Learning [2] | ICML 2026 | Published |
| Deep Learning-based Pass Intent Prediction in Football Matches | KCC 2024 | Published |

---

<h3> 🚀 Featured Projects </h3>

| Repo | Description |
| ------ | ------ |
| [zsc-ov2-lab](https://github.com/menguri/zsc-ov2-lab) | Zero-shot coordination research in MARL · Overcooked-based experiments |
| [LLM-ACL](https://github.com/lococaeco/LLM-ACL) | LLaMA 8B integrated into MuJoCo RL pipeline for reward shaping & noise control |
| [SK-AI-competition-autonomous-driving](https://github.com/menguri/SK-AI-competition-autonomous-driving) | Autonomous driving AI competition · top 30% (15/42) |

> ICML 2026 submission code will be released upon acceptance.

---

<h3> 🏆 Awards </h3>

| 주최 | 내용 | 날짜 |
| ------ | ------ | ------ |
| 한국정보과학회 (KCC) | 학부생 논문 우수상 | 2024.07 |
| 미래에셋 박현주 재단 | 해외교환장학생 선발 | 2023.08 |
| IBK 기업은행 | 우수 인턴 상 | 2023.03 |

---

<h3> 💰 Competition </h3>

| 주최 | 대회명 | 순위 | 날짜 |
| ------ | ------ | ------ | ------ |
| SK그룹 | 자율주행로봇 AI경연 | 15/42 (상위 30%) | 2025.09 |
| NH투자증권 | NH투자증권 빅데이터 경진대회 | 최종 16팀 입선 | 2022.12 |
| 한국주택토지공사 | 주차수요 예측 AI 대회 | 41/504 (상위 9%) | 2021.07 |
| G-Research | G-Research Crypto Forecasting | 607/1946 (상위 31%) | 2021.12 |

---

<h3> 🎓 Education </h3>

- **UNIST** — M.S. in Artificial Intelligence (Sep. 2025 ~ Present)
- **University of Seoul** — B.B.A. in Business Administration, GPA 3.83/4.5 (Mar. 2019 ~ Aug. 2025)

<sub>License: 투자자산운용사 💲 &nbsp;|&nbsp; ADsP</sub>
